...
+ (void)initialize
{
    checkedIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"ZYQAssetPicker.Bundle/Images/%@@2x.png",(!IS_IOS7) ? @"AssetsPickerChecked" : @"AssetsPickerChecked"]]];
    
    checkedNoIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"ZYQAssetPicker.Bundle/Images/%@@2x.png",(!IS_IOS7) ? @"No" : @"No"]]];
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    ...
    if (_indexPathsForSelectedItems.count == 10) {
        NSLog(@"最多选10张照片");
        [self.view makeToast:@"最多选10张照片"];
    }
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    NSLog(@"self.number is %ld",(long)self.number);
}
